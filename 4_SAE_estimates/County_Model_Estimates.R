...
estimates = read_excel("P:/A335/Common/SAE/Q3/HTN/Direct Estimates/County_Direct_Estimates.xlsx",
         sheet="ESTIMATES") 
names(estimates) = toupper(names(estimates)) # Capitalize all column names
estimates = estimates %>% mutate(STATE_FIPS=substr(FIPS_STATE_COUNTY,1,2))
dim(estimates)
...
estimates = estimates %>% filter(EST_TYPE == "modeled") %>% 
          mutate(STATE_FIPS = substr(FIPS_STATE_COUNTY, 1, 2))
summary(estimates)
...
# Append state estimate to county file
...
# Covariates
covariates = read_excel("P:/A335/Common/SAE/Covariates/SAE_Covariates.xlsx")  
...
IV = c("PREVALENCE")
variables = c("ACTL_PER_CAP_FFS_COST_23",       
              "STDIZD_PER_CAP_FFS_COST_23",      
              "MD_NF_ALL_GP_23",                 "MD_NF_CARD_DIS_23",              
              "HOSP_23",                         "HOSP_ADM_23",                    
              "STGH_ADULT_CARD_SVC_23",          "MEDN_AGE_20",                    
              "FORGN_BORN_POPN_PCT_23",          "URBAN_POPN_PCT_20",              
              "ISCHEMC_HEART_DIS_DETH_3YR_23",   "OTH_CARD_DIS_DETH_3YR_23",       
              "PERS_DEEP_POVTY_PCT_23",                      
              "FOOD_STAMP_RECPNT_SNAP_PCT_20",   "DISBLD_CIV_NONNSTL_18_64_PCT_23",
              "DISBLD_CIV_NONNSTL_GE65_PCT_23",  "PERS_NOINS_18_64_PCT_22",        
              "OWNER_OCCD_HOUS_PCT_20",          "POPN_DENSTY_PER_SQUR_MI_20",     
              "ANNUL_PARTCLT_MATTR_2_5_AVG_20",  "METRO",                          
              "MICRO",                           "FARMNG_DEPNDNT_TYPOLGY",         
              "MINNG_DEPNDNT_TYPOLGY",           "MFG_DEPNDNT_TYPOLGY",            
              "FED_ST_GOVT_DEPNDNT_TYPOLGY",     "RECRTN_TYPOLGY",                 
              "NONSPCLZD_DEPNDNT_TYPOLGY",       "LO_EDUCTN_TYPOLGY",              
              "LO_EMPLYMT_TYPOLGY",              "HI_POVTY_TYPOLGY",               
              "PRSTNT_POVTY_TYPOLGY",            "PRSTNT_CHILD_POVTY_TYPOLGY",     
              "POPN_LOSS_TYPOLGY",               "RETRMNT_DESTNTN_TYPOLGY",        
              "HPSA_PRIM_CARE",                  "HPSA_PRIM_CARE_WP",              
              "FOOD_INSECURITY",                
              "MORT_HEART_DISEASE",              "PRIMARY_CARE_PROVIDERS",         
              "WALK",                            "ACS_MED_HHINC",                  
...
pdf("County_Model_Plots.pdf")
sink("County_Model_Output.txt")
...
estimates_h = full_join(estimates %>% dplyr::select(-STATE) %>%
                        filter(YEAR_MONTH==months[i] & CONDITION=="HTN" & EST_TYPE=="modeled"), 
                        covariates, by="FIPS_STATE_COUNTY")
estimates_h = estimates_h %>% 
                  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
                         CONDITION="HTN", STATE_FIPS = substr(FIPS_STATE_COUNTY, 1, 2)) %>%
                  arrange(FIPS_STATE_COUNTY)
...
plot(ifelse(estimates_h$N_SAMP[is.na(estimates_h$N_SAMP)==F]>1000, 1000,
     ...
     xlab="Number of patients in county", 
...
estimates_weighted = rbind(estimates_weighted, 
                    estimates_h %>% dplyr::select(YEAR_MONTH, FIPS_STATE_COUNTY, PREVALENCE,
                                  EST_TYPE, N_SAMP, CONDITION, SE, PREVALENCE_DIRECT, STATE, 
                                  COUNTY_NAME, POPN_20_84))
...
estimates_h = estimates_h %>% 
  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
         CONDITION="HTN-C", STATE_FIPS = substr(FIPS_STATE_COUNTY, 1, 2)) %>%
  arrange(FIPS_STATE_COUNTY)
...
################### STOPPING POINT for County_Plots.R #############################

# County comparison

county = estimates_weighted %>% group_by(CONDITION, FIPS_STATE_COUNTY) %>% 
...
county_excl2019 = estimates_weighted %>% filter(substr(YEAR_MONTH,1,4) != "2019") %>%
  group_by(CONDITION, FIPS_STATE_COUNTY) %>% 
  summarize(MIN = min(PREVALENCE),
            MEDIAN = median(PREVALENCE),
            MAX = max(PREVALENCE),
            RATIO_MAX_MIN = MAX/MIN,
...
state = estimates_weighted %>% group_by(CONDITION, STATE, YEAR_MONTH) %>% 
  summarize(PREVALENCE_MODEL = weighted.mean(PREVALENCE, POPN_20_84), .groups = "drop")

state = left_join(left_join(full_join(state, state.estimates %>% 
...
sheets = list("County" = county,
              "County_excl2019" = county_excl2019,
              "State" = state,
              "Htn Corr" = state_correlation_htn,
              "Htn-C Corr" = state_correlation_htnc,
              "National" = national
) 
write.xlsx(sheets, 'County Model Output.xlsx')

# Export the full data

write.xlsx(estimates_weighted, "County_Estimates_All.xlsx",  sep=",", rowNames=F)
...
estimates_weighted = estimates_weighted %>% filter(is.na(N_SAMP)==F) %>%
  dplyr::select(-PREVALENCE_DIRECT, -STATE, -COUNTY_NAME, -POPN_20_84) %>%
...
estimates_weighted = full_join(estimates_weighted, 
                               estimates_crude %>% 
                                 dplyr::select(YEAR_MONTH, FIPS_STATE_COUNTY, CONDITION)) %>%
  mutate(EST_TYPE = "modeled")

summary(estimates_weighted)
unique(estimates_weighted$YEAR_MONTH)
unique(substr(estimates_weighted$FIPS_STATE_COUNTY,1,2))
unique(estimates_weighted$FIPS_STATE_COUNTY)
...
estimates_crude = left_join(estimates_crude, estimates_suppressed %>% 
                              dplyr::select(YEAR_MONTH, FIPS_STATE_COUNTY, CONDITION, SUPPRESS)) %>%
  rename(N_SAMP_OLD = N_SAMP, PREVALENCE_OLD = PREVALENCE, SE_OLD = SE) %>%
...
estimates_crude = estimates_crude %>% dplyr::select(-SUPPRESS, -STATE, -STATE_FIPS)

# Final data file
estimates_final = rbind(estimates_weighted, estimates_crude) %>% 
  mutate(NPATS = round(N_SAMP)) %>%
  arrange(EST_TYPE, CONDITION, YEAR_MONTH, FIPS_STATE_COUNTY) %>%
  dplyr::select(-N_SAMP)

summary(estimates_final)
unique(estimates_final$YEAR_MONTH)
unique(substr(estimates_final$FIPS_STATE_COUNTY,1,2))
unique(estimates_final$FIPS_STATE_COUNTY)
unique(estimates_final$EST_TYPE)
unique(estimates_final$CONDITION)

# Export data

write.xlsx(estimates_final, "County_Estimates_Modeled.xlsx",  sep=",", rowNames=F)
