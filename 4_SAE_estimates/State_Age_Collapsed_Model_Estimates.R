...
estimates = read_excel("P:/A335/Common/SAE/Q3/HTN/Direct Estimates/State_Age_Collapsed_Direct_Estimates.xlsx",
         sheet="ESTIMATES") 
...
estimates_crude <- estimates_crude %>%
  ...
  arrange(N_SAMP, AGEC_COL, .by_group = TRUE) %>%
  mutate(
...
trim.n = estimates %>% group_by(YEAR_MONTH, EST_TYPE, CONDITION, AGEC_COL) %>%
            summarise(TRIM_N_SAMP = quantile(N_SAMP, trim.q), .groups = "drop")

estimates = left_join(estimates, trim.n, by=c("YEAR_MONTH", "EST_TYPE", "CONDITION", "AGEC_COL")) %>%
...
# Covariates
covariates = read_excel("P:/A335/Common/SAE/Covariates/State_Age_Collapsed_Covariates.xlsx")  
...
covariates <- dummy_cols(covariates, 
                         select_columns = "AGEC_COL", 
                         remove_first_dummy = FALSE, 
                         remove_selected_columns = FALSE)

# clean names
names(covariates) <- gsub("-", "_", names(covariates))
names(covariates) <- gsub("AGEC_COL_", "AGE_", names(covariates))

# Check
covariates %>% group_by(AGEC_COL, AGE_20_44, AGE_45_64, AGE_65_84) %>% count()
...
pdf("State_Age_Collapsed_Model_Plots.pdf")
sink("State_Age_Collapsed_Model_Output.txt")
...
estimates_h = full_join(estimates %>% 
                  filter(YEAR_MONTH==months[i] & CONDITION=="HTN" &  EST_TYPE=="modeled"), 
                        covariates, by=c("STATE_FIPS","AGEC_COL"))
estimates_h = estimates_h %>% 
                  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
                         CONDITION="HTN") %>% arrange(STATE_FIPS, AGEC_COL)
...
estimates_h = estimates_h %>% 
  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
         CONDITION="HTN-C") %>% arrange(STATE_FIPS, AGEC_COL)
...
state = estimates_weighted %>% group_by(CONDITION, STATE, AGEC_COL) %>% 
...
state_excl2019 = estimates_weighted %>% filter(substr(YEAR_MONTH,1,4) != "2019") %>%
  group_by(CONDITION, STATE, AGEC_COL) %>% 
  summarize(MIN = min(PREVALENCE),
            MEDIAN = median(PREVALENCE),
            MAX = max(PREVALENCE),
            RATIO_MAX_MIN = MAX/MIN,
...
write.xlsx(sheets, 'State Age Collapsed Model Output.xlsx')

# Export the full data

write.xlsx(estimates_weighted, "State_Age_Collapsed_Estimates_All.xlsx",  sep=",", rowNames=F)
...
estimates_crude = left_join(estimates_crude, estimates_suppressed %>% 
                dplyr::select(YEAR_MONTH, STATE_FIPS, AGEC_COL, CONDITION, SUPPRESS)) %>%
  rename(N_SAMP_OLD = N_SAMP, PREVALENCE_OLD = PREVALENCE, SE_OLD = SE) %>%
...
estimates_final = rbind(estimates_weighted, estimates_crude) %>% 
                    rename(NPATS = N_SAMP) %>%
                    arrange(EST_TYPE, CONDITION, YEAR_MONTH, STATE_FIPS, AGEC_COL)
  
summary(estimates_final)
unique(estimates_final$YEAR_MONTH)
unique(estimates_final$AGEC_COL)
...
write.xlsx(estimates_final, "State_Age_Collapsed_Estimates_Modeled.xlsx",  sep=",", rowNames=F)
