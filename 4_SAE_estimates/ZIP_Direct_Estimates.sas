...
else if substr(year_month,1,4) in("2025","2026") then file_flag = 4;
...
outfile="P:\A335\Common\SAE\Q3\HTN\Direct Estimates\ZIP_Direct_Estimates.csv"
dbms=csv
replace;
run;
...
proc export data = estimates (where=(est_type="modeled" & file_flag = 1))
			outfile = "P:\A335\Common\SAE\Q3\HTN\Direct Estimates\ZIP_Direct_Estimates.xlsx"
			dbms = excel replace;
...
proc export data = estimates (where=(est_type="modeled" & file_flag = 4))
			outfile = "P:\A335\Common\SAE\Q3\HTN\Direct Estimates\ZIP_Direct_Estimates.xlsx"
			dbms = excel replace;
			sheet = "modeled 2025";
run;

proc export data = estimates (where=(est_type="crude" & file_flag = 4))
			outfile = "P:\A335\Common\SAE\Q3\HTN\Direct Estimates\ZIP_Direct_Estimates.xlsx"
			dbms = excel replace;
			sheet = "crude 2025";
run;
