...
estimates = read_excel("P:/A335/Common/SAE/Q3/HTN/Direct Estimates/State_Race_Direct_Estimates.xlsx",
         sheet="ESTIMATES") 
...
estimates_crude <- estimates_crude %>%
  ...
  arrange(N_SAMP, RACEETH2, .by_group = TRUE) %>%
  mutate(
...
trim.n = estimates %>% group_by(YEAR_MONTH, EST_TYPE, CONDITION, RACEETH2) %>%
            summarise(TRIM_N_SAMP = quantile(N_SAMP, trim.q), .groups = "drop")

estimates = left_join(estimates, trim.n, by=c("YEAR_MONTH", "EST_TYPE", "CONDITION", "RACEETH2")) %>%
...
# Covariates
covariates = read_excel("P:/A335/Common/SAE/Covariates/State_Race_Covariates.xlsx")  
...
pdf("State_Race_Model_Plots.pdf")
sink("State_Race_Model_Output.txt")
...
estimates_h = full_join(estimates %>% 
                  filter(YEAR_MONTH==months[i] & CONDITION=="HTN" &  EST_TYPE=="modeled"), 
                        covariates, by=c("STATE_FIPS","RACEETH2"))
estimates_h = estimates_h %>% 
                  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
                         CONDITION="HTN") %>% arrange(STATE_FIPS, RACEETH2)
...
estimates_h = estimates_h %>% 
  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
         CONDITION="HTN-C") %>% arrange(STATE_FIPS, RACEETH2)
...
state = estimates_weighted %>% group_by(CONDITION, STATE, RACEETH2) %>% 
...
state_excl2019 = estimates_weighted %>% filter(substr(YEAR_MONTH,1,4) != "2019") %>%
  group_by(CONDITION, STATE, RACEETH2) %>% 
  summarize(MIN = min(PREVALENCE),
            MEDIAN = median(PREVALENCE),
            MAX = max(PREVALENCE),
            RATIO_MAX_MIN = MAX/MIN,
...
write.xlsx(sheets, 'State Race Model Output.xlsx')

# Export the full data

write.xlsx(estimates_weighted, "State_Race_Estimates_All.xlsx",  sep=",", rowNames=F)
...
estimates_crude = left_join(estimates_crude, estimates_suppressed %>% 
                dplyr::select(YEAR_MONTH, STATE_FIPS, RACEETH2, CONDITION, SUPPRESS)) %>%
  rename(N_SAMP_OLD = N_SAMP, PREVALENCE_OLD = PREVALENCE, SE_OLD = SE) %>%
...
estimates_final = rbind(estimates_weighted, estimates_crude) %>% 
                    rename(NPATS = N_SAMP) %>%
                    arrange(EST_TYPE, CONDITION, YEAR_MONTH, STATE_FIPS, RACEETH2)
...
write.xlsx(estimates_final, "State_Race_Estimates_Modeled.xlsx",  sep=",", rowNames=F)
