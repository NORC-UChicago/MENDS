...
estimates = read_excel("P:/A335/Common/SAE/Q3/HTN/Direct Estimates/State_Direct_Estimates.xlsx",
         sheet="ESTIMATES") 
...
trim.n = estimates %>% group_by(YEAR_MONTH, EST_TYPE, CONDITION) %>%
            summarise(TRIM_N_SAMP = quantile(N_SAMP, trim.q), .groups = "drop")

estimates = left_join(estimates, trim.n, by=c("YEAR_MONTH", "EST_TYPE", "CONDITION")) %>%
...
estimates = estimates %>% 
              mutate(PREVALENCE_DIRECT = PREVALENCE, PREVALENCE = PREVALENCE/100,
                     SE_INIT = if_else(EST_TYPE == "modeled",
                       sqrt(PREVALENCE*(1-PREVALENCE)*(1+(CV/100)^2)/N_SAMP_ADJ), NA))
summary(estimates)
...
# Covariates
covariates = read_excel("P:/A335/Common/SAE/Covariates/State_Covariates.xlsx")  
...
################### STOPPING POINT for State_Plots.R #############################
...
pop = read_excel("P:/A335/Common/SAE/Covariates/SAE_Covariates.xlsx") %>% 
