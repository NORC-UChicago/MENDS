...
estimates = read_excel("P:/A335/Common/SAE/Q3/HTN/Direct Estimates/State_Sex_Direct_Estimates.xlsx",
         sheet="ESTIMATES") 
...
estimates_crude <- estimates_crude %>%
  ...
  arrange(N_SAMP, SEX, .by_group = TRUE) %>%
  mutate(
...
trim.n = estimates %>% group_by(YEAR_MONTH, EST_TYPE, CONDITION, SEX) %>%
            summarise(TRIM_N_SAMP = quantile(N_SAMP, trim.q), .groups = "drop")

estimates = left_join(estimates, trim.n, by=c("YEAR_MONTH", "EST_TYPE", "CONDITION", "SEX")) %>%
...
# Covariates
covariates = read_excel("P:/A335/Common/SAE/Covariates/State_Sex_Covariates.xlsx")  
...
# Check
covariates %>% group_by(SEX, SEX_Male, SEX_Female) %>% count()
...
pdf("State_Sex_Model_Plots.pdf")
sink("State_Sex_Model_Output.txt")
...
estimates_h = full_join(estimates %>% 
                  filter(YEAR_MONTH==months[i] & CONDITION=="HTN" &  EST_TYPE=="modeled"), 
                        covariates, by=c("STATE_FIPS","SEX"))
estimates_h = estimates_h %>% 
                  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
                         CONDITION="HTN") %>% arrange(STATE_FIPS, SEX)
...
estimates_h = estimates_h %>% 
  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
         CONDITION="HTN-C") %>% arrange(STATE_FIPS, SEX)
...
state = estimates_weighted %>% group_by(CONDITION, STATE, SEX) %>% 
...
write.xlsx(sheets, 'State Sex Model Output.xlsx')

# Export the full data

write.xlsx(estimates_weighted, "State_Sex_Estimates_All.xlsx",  sep=",", rowNames=F)
...
estimates_crude = left_join(estimates_crude, estimates_suppressed %>% 
                dplyr::select(YEAR_MONTH, STATE_FIPS, SEX, CONDITION, SUPPRESS)) %>%
  rename(N_SAMP_OLD = N_SAMP, PREVALENCE_OLD = PREVALENCE, SE_OLD = SE) %>%
...
estimates_final = rbind(estimates_weighted, estimates_crude) %>% 
                    rename(NPATS = N_SAMP) %>%
                    arrange(EST_TYPE, CONDITION, YEAR_MONTH, STATE_FIPS, SEX)
  
summary(estimates_final)
unique(estimates_final$YEAR_MONTH)
unique(estimates_final$SEX)
...
write.xlsx(estimates_final, "State_Sex_Estimates_Modeled.xlsx",  sep=",", rowNames=F)
