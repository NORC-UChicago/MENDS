/********************************************************************/
/***PROGRAM: 3_0_Quickstart_MENDS - county est - 2022.SAS		  ***/
/***VERSION: 1.0 									 			  ***/
...
/***OBJECTIVE: PROGRAM COMPUTES WEIGHTED COUNTY ESTIMATES FOR 2022***/
/********************************************************************/
...
/***/%LET START_YEAR = 2022; /**NOTE: numeric year for start value*/
/***/%LET END_YEAR = 2022; /**NOTE: numeric year for end value*/
/***/%LET START_MONTH = 1; /**NOTE: numeric year for start value*/
/***/%LET END_MONTH = 12; /**NOTE: numeric month for end value*/
...
%macro outputloop();
	...
	Proc Printto Log="&OUTFILEPATH.\SAS LOG\&OUTFOLDER.\5_Estimates_county_&START_YEAR.&START_MONTH._&END_YEAR.&END_MONTH._&DateTime..log" New;
	Run;
		
	ods listing file="&OUTFILEPATH.\SAS LOG\&OUTFOLDER.\5_Estimates_county_&START_YEAR.&START_MONTH._&END_YEAR.&END_MONTH._&DateTime..lst";	

	/*@Action: clear appended files to make sure we have clean files*/
	proc datasets library=sasout nolist;
		delete excl_county_&START_YEAR.&START_MONTH._&END_YEAR.&END_MONTH.
			est_county_&START_YEAR.&START_MONTH._&END_YEAR.&END_MONTH.
			Est_wide_county_&START_YEAR.&START_MONTH._&END_YEAR.&END_MONTH.
			supr_county_&START_YEAR.&START_MONTH._&END_YEAR.&END_MONTH.;
	run;

	quit;
		
	/*@Action: Declare the county specifc macros*/
	%Include "&PROGFILEPATH.\3_1_exclusions - county.sas" / LRECL = 1000;
	%Include "&PROGFILEPATH.\3_2_estimates - county.sas" / LRECL = 1000;
	%Include "&PROGFILEPATH.\3_3_suppress - county.sas" / LRECL = 1000;
	...
	%do est_loop=&min. %to &max.;	
		...
		/*@Action: Loop through county level estimates ***/
		proc sql noprint;
	...
	data Est_crd (rename=(Sample_PT=Npats Crude_Prev=prevalance Crude_StdErr=se));
		set sasout.Est_wide_county_&START_YEAR.&START_MONTH._&END_YEAR.&END_MONTH.;
	...
	data sasout.Est_county_&START_YEAR.&START_MONTH._&END_YEAR.&END_MONTH.;
