...
estimates = read_excel("P:/A154/Common/SAE/Q1/Direct Estimates/State_Direct_Estimates.xlsx",
         sheet="ESTIMATES") 
...
estimates = estimates %>% 
              mutate(PREVALENCE_DIRECT = PREVALENCE, PREVALENCE = PREVALENCE/100,
                     SE_INIT = if_else(EST_TYPE == "modeled",
                       sqrt(PREVALENCE*(1-PREVALENCE)*(1+(CV/100)^2)/N_SAMP), NA))
summary(estimates)
...
estimates_model = estimates_model %>% 
        mutate(GVF_SE = 
                 GVF.smooth(n=estimates_model$N_SAMP, 
                            p=estimates_model$PREVALENCE, y=estimates_model$SE_INIT))
...
# Model fit
temp = nlminb(start=median(estimates_model$GVF_SE^2), obj=sigma.FH, lower=0.000001, upper=Inf, 
              y=estimates_model$PREVALENCE, psi=estimates_model$GVF_SE^2, X=X)
cat("\n Convergence \n")
print(temp)
...
estimates_h = estimates_h %>% 
  mutate(YEAR_MONTH=months[i], EST_TYPE="modeled", 
         CONDITION="HTN-C") %>% arrange(STATE_FIPS)

estimates_cor = estimates_h %>% filter(N_SAMP >=30 & is.na(N_SAMP)==F) # Used for deriving correlations
...
temp = nlminb(start=median(estimates_model$GVF_SE^2), 
              obj=sigma.FH, lower=0.000001, upper=Inf, y=estimates_model$PREVALENCE, 
              psi=estimates_model$GVF_SE^2, X=X)
cat("\n Convergence \n")
print(temp)
cat("\n Weight for the Direct Estimate \n")
...
state = estimates_weighted %>% group_by(CONDITION, STATE) %>% 
...
state2 = full_join(full_join(estimates_weighted %>% dplyr::select(-EST_TYPE, -SE, -N_SAMP) %>%
                                         rename(PREVALENCE_MODEL = PREVALENCE), 
...
pop = read_excel("P:/A154/Common/SAE/Covariates/SAE_Covariates.xlsx") %>% 
              dplyr::select(STATE, popn_20_84) %>%
              group_by(STATE) %>% 
              summarise(POPN_20_84 = sum(popn_20_84))

national = left_join(estimates_weighted, pop) %>% group_by(CONDITION, YEAR_MONTH) %>% 
...
write.xlsx(sheets, 'State Model Output.xlsx')

# Export the full data

write.xlsx(estimates_weighted, "State_Estimates_All.xlsx",  sep=",", rowNames=F)
...
estimates_weighted = full_join(estimates_weighted, 
                      estimates_crude %>% 
                        dplyr::select(YEAR_MONTH, STATE_FIPS, CONDITION)) %>%
...
estimates_crude = left_join(estimates_crude, estimates_suppressed %>% 
                              dplyr::select(YEAR_MONTH, STATE_FIPS, CONDITION, SUPPRESS)) %>%
  rename(N_SAMP_OLD = N_SAMP, PREVALENCE_OLD = PREVALENCE, SE_OLD = SE) %>%
...
estimates_final = rbind(estimates_weighted, estimates_crude) %>% 
                    rename(NPATS = N_SAMP) %>%
                    arrange(EST_TYPE, CONDITION, YEAR_MONTH, STATE_FIPS)
...
write.xlsx(estimates_final, "State_Estimates_Modeled.xlsx",  sep=",", rowNames=F)
